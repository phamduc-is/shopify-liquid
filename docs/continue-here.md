...
5. **Bước 4 — Build từng trang theo đúng thứ tự roadmap: Header/Footer (13-14) → Homepage → Product Page (15-16) → Category (17) → Cart (18)** — **CHƯA LÀM, làm tiếp từ đây**. Mỗi section vẫn theo phương pháp đã luyện: **HTML tĩnh (dữ liệu giả) → thay dần bằng Liquid động (dùng kỹ thuật `render for as` — Ngày 6 — để loop `product-card` qua danh sách sản phẩm) → thêm `{% schema %}` cuối cùng, thêm dần từng setting một, test bằng Theme Editor sau mỗi lần thêm**.
...
Bắt đầu từ **Bước 4** — build Header/Footer (Ngày 13–14) trước, vì đây là phần được dùng ở mọi trang. Theo đúng phương pháp đã luyện: HTML tĩnh (data giả) → thay dần Liquid động (dùng `render for as` để loop `snippets/product-card.liquid` khi cần, ví dụ menu/mega-menu) → thêm `{% schema %}` cuối cùng, thêm dần từng setting một, test bằng Theme Editor sau mỗi lần thêm. Snippet dùng chung (`product-card`, `rating-stars`) đã có sẵn từ Bước 3, chưa cần tạo lại — chỉ còn thiếu bước wire vào section thật (sẽ tự hết 2 warning `OrphanedSnippet`).
